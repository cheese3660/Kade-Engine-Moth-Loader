...
class LuaBase
{
    ...
    public function add_callback(function_name: String, func: Dynamic) {
        Lua_helper.add_callback(lua, function_name, func);
    }

    public function new(lua_path:String)
    {
        lua = LuaL.newstate();
        LuaL.openlibs(lua);
        Lua.init_callbacks(lua);
        var result = LuaL.dofile(lua, lua_path);
        if (result != 0)
        {
            Application.current.window.alert("Lua compile error:\n" + Lua.tostring(lua,result), "Lua Loader");
            lua = null;
            LoadingState.loadAndSwitchState(new MainMenuState());
        }
    }
